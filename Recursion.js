    ...
    if (maze[y][x]==='e'){
     ...
     if (y+1 <maze.length){
         if( maze[y+1][x] !=='*' && !visited.includes(`[${y+1}][${x}]`)){
             console.log(x,y)
             return Maze(maze, x, y+1, [...visited], steps+'D')
         }
     }
     if (x+1 < maze[y].length){
         if(maze[y][x+1] !=='*' && !visited.includes(`[${y}][${x+1}]`)){
            console.log(x,y)
             return Maze(maze, x+1, y, [...visited], steps+'R')
         }
     }
     if(y-1 >=0){
         if(maze[y-1][x] !=='*' && !visited.includes(`[${y-1}][${x}]`)){
            console.log(x,y)
             return Maze(maze, x, y-1, [...visited], steps+'U')
         }
     ...
     if(x-1 >= 0){
        if(maze[y][x-1] !=='*' && !visited.includes(`[${y}][${x-1}]`)) {
            console.log(x,y)
            return Maze(maze, x-1, y, [...visited], steps+'L')
        }
